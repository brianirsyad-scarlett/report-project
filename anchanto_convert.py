...
# ========================= CONFIGURATION =========================
SOURCE_FOLDER_ID = "1TT0eJXx_Mz8lQiM0M8omBHpzvl4troeg"
DEST_FOLDER_ID = "1FxQI2yMEq4wTwRrxH2aaTi6otzrkJl9v"
...
# Build credentials using the refresh token and client secrets
creds = Credentials(
    token=None,
...
# Refresh the token (obtains a new access token)
creds.refresh(Request())

# Authorise Google APIs
...
def get_processed_timestamps(folder_id):
    """Return dict of file_id -> last_modified_time from the JSON file."""
...
def save_processed_timestamps(folder_id, timestamps):
    """Save the timestamps dict to the JSON file in the given folder."""
...
def split_excel_to_sheets(file_id, file_name, dest_folder_id):
    """Convert an Excel file into Google Sheets, split by ROWS_PER_SHEET."""
    # Download and read
    ...
    # Keep only first 16 columns (A–P)
    if df.shape[1] > 16:
        df = df.iloc[:, :16]
    ...
    for part_num in range(1, parts + 1):
        ...
        # Write in batches of 100,000 rows
        batch_size = 100000
